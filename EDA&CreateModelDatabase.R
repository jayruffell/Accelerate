...
# 1. Read in Placements match table and add in info about ad type (RMK vs BROAD; and route-specific ads).
...
route <- 'LH' # allRoutes' # 'TAS', 'DOM', 'PI', 'LH', 'allRoutes' # NB could use a SourceAllScripts.R script to loop through all routes - see classifyURLsByDomainName&ConvProbForURLByAdPageCategory.r for an e.g.
...
negativeInstanceDownsampling <- 0.001 # randomly downsample this proportion of nonconvs / non-lastclick conversions
...
cat(paste0('\nCREATING MODEL DATABASE: \n dataset=', dataset, ' // route=', route, ' // output database=', outputDatabase, ' // C/I lookback windows=', lastClickWindow, '/', lastImpWindow, ' // date range=', firstDate, '-', maxDate,'\n'))
...
# #__________________________________________________________________________________________________________________________________
# 
# # EDIT - MOVED TO CLUSTERING AND MODELLIGN SCRIPT
# 

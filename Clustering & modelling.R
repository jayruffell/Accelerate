...
if(outputDatabase=='df_oneRowPerAdAndLastClickConversionResponse'){
  ...
  warning('Renaming "lastClickConversion" var to "Converter", Just so theres a single name for the target var thru rest of code. \n')
}
df$Converter <- as.factor(ifelse(df$Converter==1, 'y', 'n')); warning(' Changing conv/nonconv values from 1/0 to y/n for modelling\n')
...
warning(' NB make sure any new character vars are converted to factors to avoid potential errors with random forest\n')
...
#2. Set up cross validation training control - timeSlice vs kfold ----
#__________________________________________________________________________________________________________________________________

if(CVMethod=='timeSlice'){
  ...
  control <- trainControl(method="repeatedcv", number=10, repeats=5,
                          ...
                          classProbs=TRUE,
                          sampling='down') # downsample within each resampling fold, then test on full holdout fold. https://topepo.github.io/caret/subsampling-for-class-imbalances.html for details
} else { cat(' WARNING - check CVMethod value\n')}
...
if(compareModels=='Y'){
  ...
  # # TEMP - try adding adX:daypart interaction (cos e.g. ppl read new in the morning, entertainment in the afternoon etc). HASHED OUT - WORSE PERFORMANCE
  # interactionTerms <- paste(unname(sapply(colnames(modelDF)[grepl('adX_', colnames(modelDF))], function(x) paste0(x, ':daypart + '))), collapse="")
  # interactionTerms <- substr(interactionTerms, start=1, stop=nchar(interactionTerms)-3) # removing final plus at end
  # myFormula <- as.formula(paste0('Converter ~ . + ', interactionTerms))
  # mGlmnet <- train(myFormula, data=modelDF, method="glmnet", trControl=control, metric='ROC')
  ...
  mXGB <- train(Converter~., data=modelDF, method="xgbTree", trControl=control, metric='ROC') # XGB is supposed to be hot shit, tho apparently caret defaults aren't great, xgboost defaults are better; see here https://github.com/dmlc/xgboost/issues/1371
...
# #__________________________________________________________________________________________________________________________________
# 
# # 4.b. Feature selection methods - test performance of random forest with feature selection [*** HASHING OUT - this is just a placeholder; can do later if I want to explore RF's further ----
#__________________________________________________________________________________________________________________________________
...
# # 4.c. Feature selection methods -  try recursive feature elimination on a random forest model & bagged tree model to look at which vars I should include [*** Hashing out - takes ages, and I can return to this later if needs be ***] ----
# #__________________________________________________________________________________________________________________________________
...
if(modelType=='tree' | modelType=='both'){
  ...
  treeSummaryDF <- treeSummaryDF %>%
    ...
    mutate(CPA=round(TotalRevenueNZD/numConversions, 2))
  ...
  warning('numDBMAds column may need to be calculated differently if outputDatabase is based on one row per user cf one row per ad. At very least name will need to change name to numDBMUsers or similar\n')
...
if(modelType=='lasso' | modelType=='both'){
  ...
  warning('Joining segments to df to calculate cost, numRows etc - this means that combinations of predictors with the highest lift may not make it into the final segments, cos this combination didnt exist in reality.\n')
